 ...
 */
	...
	// Parallelized for faster initialization given the potentially long loops
...
#pragma omp parallel for
	...
	// Parallelized using the thread_count argument.
	// Buffer reduction is used on seq_matches to remove dependencies.
	// pat_matches is also reduced, being a simple counter.
	// Per the data gathered during testing, static scheduling is chosen.
	int pat_ind;
#pragma omp parallel for num_threads(thread_count) reduction(+: seq_matches[:seq_length]) reduction(+: pat_matches) schedule(static, pat_number/thread_count)
	for( pat_ind=0; pat_ind < pat_number; pat_ind++ ) {
		...
		/* 5.2. Pattern found */
		if ( pat_found[pat_ind] != (unsigned long)NOT_FOUND ) {
			/* 5.2.1. Increment the number of pattern matches on the sequence positions */

			// This section used to be a function but was unrolled here since this is the only usecase.
			// This was done also to avoid potential parallelization issues and unnecessary stack loading. 
			
			unsigned long i;

			for (i = 0; i < pat_length[pat_ind]; i++) {
					//#pragma omp atomic
					seq_matches[pat_found[pat_ind] + i]++;
			}
		}
	}
	...
	// Checksum calculation parallelized using reduction
...
#pragma omp parallel for reduction(+: checksum_matches)
	...
	// Due to reduction, the checksums might get out of range so we need to bring them in the range once more.
